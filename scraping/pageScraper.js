    ...
    async scraper(browser){
        ...
        async function scrapeCurrentPage(){
            ...
            let pagePromise = (link) => new Promise(async(resolve, reject) => {
                ...
                dataObj['location'] = await newPage.$eval('#pb-job-location', text => text.textContent);
                ...
                dataObj['description'] = await newPage.$eval('.job-description', text => text.textContent);
                dataObj['date'] = await newPage.$eval('[translate="section-jobb-about.published"]', text => text.textContent);
            ...
            fs.writeFile("./public/data/data.json", JSON.stringify(scrapedData), (err) => {
                if (err)
                    console.log(err);
                else {
